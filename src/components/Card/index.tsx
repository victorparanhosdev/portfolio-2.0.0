        ...
        <div {...props} className="w-full">
          ...
          <h1 className="flex items-center justify-center w-full h-full text-xl font-bold text-gray-500">
            ...
            <div className="max-[330px]:grid mt-10 flex max-[330px]:gap-4 gap-6">
                {projetos?.homepage && <a href={projetos?.homepage} target="_blank"><Button iconLeft={Link} state="primary">Visualizar</Button></a>}
                {projetos?.html_url && <a href={projetos?.html_url} target="_blank"><Button iconLeft={GithubLogo} state="outline">Código</Button></a>}
            </div>
        </div>
    )
}